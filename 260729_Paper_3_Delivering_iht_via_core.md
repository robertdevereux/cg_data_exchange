...
**Marital status and ownership work together, once, per asset.** Every asset an executor records is asked about exactly once. Ownership type doesn't just determine the deceased's share of an asset's value — it determines the *mechanism* by which the asset passes on, and that mechanism differs by ownership type: sole ownership and shared ownership in defined shares pass under the will or intestacy rules; joint ownership with automatic survivorship passes to the other joint owner(s) regardless of the will. Where the deceased was married and this is the first death, establishing whether an asset (or part of it) passed to the surviving spouse follows from that same mechanism — a direct question for the former, a simple derivation from ownership data already collected for the latter. Wherever the answer is that the asset passed entirely to the spouse, the executor's work on it ends there — no valuation evidence, since no tax turns on getting the figure precisely right. Everything else — including every asset in an estate with no surviving spouse, and the non-spouse-bound remainder of any partly-exempt asset — gets the fuller treatment: valuation evidence, using a shape that's identical across property, bank accounts, shares, and business interests, varying only in the fine detail of what counts as evidence for each.

Because every one of these answers is persisted the moment it's confirmed, the home page can also offer executors a live, simple summary of the estate as it stands — gross, taxable, and probate-relevant figures, built entirely from asset rows the executor has already completed, with nothing assumed or estimated for work still in progress.
