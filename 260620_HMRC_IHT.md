...
Date: 20 June 2026
Status: Current — reflects code as at end of sprint 4
...
if not returning_from_core:          ← ENTRY
    ...
    common_assets    → _entry_common_assets()
    None             → _render_home()
    ↓
if returning_from_core:              ← EXIT
    ...
    common_assets    → (nothing) → fall through
    ↓
...
`_entry_tailor`:
1. `_enter_core` — sets `iht_in_core = True`
2. `call_core([{'type': 'section', 'id': sid} for sid in TRIAGE_SECTION_IDS], title='Tailor your submission')` —
   shows a filtered section list page with S4, S5, S6

Because `call_core` receives multiple items, it writes `permitted_section_ids`
and `top_level_items` to session and routes to `regime_top_level` (the core
mixed top-level list view). The user sees a titled page listing the three
...
## 8a. Action button: Common assets (and pattern for pensions, other)
...
Adding a fourth triage set: add one entry to `TRIAGE_SETS`, add question→schedule
entries to `QUESTION_SCHEDULE_MAP`, add action view and URL. Everything else
(home page rows, progressive URL logic) follows automatically.
...
`_entry_common_assets`:
1. Calls `_get_built_schedule_items` for HMRC_S4
2. If empty → returns None → falls through to home (no navigation)
3. `_enter_core` — sets `iht_in_core = True`
4. `call_core(items, title='Common assets', url_prefix='hmrc')`
5. Redirects to entry URL (schedule list or direct to single schedule's sections)

### Exit

`_exit_common_assets` — nothing to do. Falls through to `_render_home`.

### Pattern for pensions and other

`iht_action_pensions` / `iht_action_other` and their entry/exit functions
follow the identical pattern — not yet built (D2).
...
| Item | Backlog ref |
|------|-------------|
| Pensions and other asset buttons (same pattern as common assets) | D2 |
