...
3. **Duplicate found** → the draft's `Answer` rows (owned by the actor) are
   ...
   future estates.
4. **Unique** → `_promote_case_to_verified(case, actor, deceased_name)`:
   - Creates a new, **inactive** `User` (`username='ihtsubject_<case pk>'`,
     `set_unusable_password()`, no email needed) representing the deceased,
     using the name just captured in HMRC_1.
   - Re-keys the case's already-written `Answer`, `AnswerHistory`, and
     `SectionStatus` rows from the actor to this new `User` (all six
     `request.user`→`case.user` sites fixed in `views_layer2.py` on 5 July
     2026 — see Core Platform Reference section 4a — depend on this having
     happened correctly).
   - Sets `case.user` to the new deceased identity and assigns the IHT
     reference.
...
| Item | Backlog ref |
|------|-------------|
| Reckoner parts 1 and 2 (married, widowed) — also needs re-checking against HMRC_14/43's new two-question shape | D1 |
...
| `dept_dwp` migrate to shared `choose_user_for_regime` | new, 5 July 2026 |
