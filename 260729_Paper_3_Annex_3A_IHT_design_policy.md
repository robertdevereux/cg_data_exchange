...
## Idea 2: Ownership tells you how an asset passes, not who it passes to — and the marital-status question follows from that
...
Ownership type doesn't, on its own, tell you who inherits an asset. It tells you the *mechanism* by which the destination is decided, and that mechanism differs by ownership type:

- **Sole ownership and shared ownership in defined shares (tenants in common)** pass under the deceased's will, or under the intestacy rules if there's no will — a fact about that person's wishes, or the law's default, independent of who else (if anyone) held a share.
- **Joint ownership with automatic survivorship (joint tenants)** passes automatically to the other joint owner(s), regardless of what any will says — the destination isn't a separate fact to ask about at all; it's already implied by who the other owner(s) were.

This matters because of what happens next. **Where the deceased was married, and this is the first death of the couple**, assets that pass entirely to the surviving spouse attract no Inheritance Tax regardless of their value — spouse exemption applies. For these assets, the executor needs only a light pass: enough to establish what was owned and its value, so probate is accurate and HMRC has a reliable total estate figure, but nothing more. There's no need for detailed valuation evidence, because no tax turns on getting that value exactly right. But *establishing whether an asset is spouse-bound* has to be done differently depending on which of the two mechanisms above applies — a will/intestacy question for sole ownership and tenants in common; simply confirming whether the spouse was the other joint owner, for joint tenants, since the arithmetic then works out the rest without a further question.

**Every asset that doesn't pass entirely to the spouse — including everything in an estate where there is no surviving spouse at all (single, divorced, or the second death of a widowed person), and including the taxable remainder of an asset that only passed *partly* to the spouse — needs the fuller pass.** This is where ownership detail, valuation evidence, and (where relevant) substitute questions in the absence of a professional valuation all come into play, because these are exactly the assets where the value recorded actually determines the tax due.

The technical detail of how three ownership-specific question sequences achieve all of this — without asking the executor the same question twice, and without building two separate journeys through the same asset class — is set out in Annex 3B. At the policy level, the important point is simply this: **every asset gets asked about once**, and the depth of what follows depends only on where it's going. An estate passing entirely to a spouse involves almost no further questions at all. An estate with even one asset going wholly or partly to someone else gets the full treatment for that asset (or that portion of it), and only that asset.
...
## Idea 3: the executor can see where the estate stands, at any point

Because every question answered through the utility is a confirmed, persisted answer — not a disposable form field — the home page can offer executors something the current process can't: a running view of the estate as it stands, built entirely from what's already been recorded. A button on the home page, "View estate as so far defined," shows a simple table — one row per asset class, with the deceased's share of the value shown three ways: the gross figure (before any exemption), the taxable figure (after spouse exemption), and the figure relevant to probate itself. Each asset class can be expanded to show the same three figures asset by asset.

This costs nothing beyond what the design already does. It reads back rows the executor has already completed and adds them up the same way the system does anywhere else it needs a total. Nothing in progress, half-finished, or abandoned partway through shows up — the view only ever reflects assets the executor has genuinely finished entering, which keeps it simple and keeps it honest: it's progress so far, nothing more claimed.
